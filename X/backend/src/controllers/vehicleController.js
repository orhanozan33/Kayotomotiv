...
import { supabaseApi } from '../config/database-api.js';
...
export const getVehicles = async (req, res, next) => {
  try {
    ...
    try {
      ...
      vehicles = await supabaseApi.getVehicles(filters);
      console.log('✅ Supabase API: Found', vehicles.length, 'vehicles');
    } catch (supabaseError) {
      console.warn('⚠️  Supabase API failed, trying PostgreSQL pool:', supabaseError.message);
      vehicles = await Vehicle.findAll(filters);
      console.log('✅ PostgreSQL pool: Found', vehicles.length, 'vehicles');
    }
    ...
    const vehiclesWithImages = await Promise.all(
      vehicles.map(async (vehicle) => {
        try {
          let images = [];
          try {
            images = await supabaseApi.getVehicleImages(vehicle.id);
          } catch (imageError) {
            console.warn('⚠️  Supabase API image fetch failed, trying PostgreSQL:', imageError.message);
            images = await Vehicle.getImages(vehicle.id);
          }
