# Tam Sistem Yedekleme Script'i
# Kod, DB Schema, Backend - Her şeyi içerir

$destPath = "C:\Users\orhan\OneDrive\Masaüstü\oto tamir"
...
Write-Host "TAM SISTEM YEDEKLEME BASLIYOR" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
# Root files (backup script'leri hariç)
$rootFiles = Get-ChildItem -Path . -File | Where-Object {
    ...
    $_.Name -ne "check-backup.ps1" -and $_.Name -ne "backup-full.ps1"
}
...
# Directories (node_modules, .next, .git hariç)
...
foreach ($item in $allItems) {
    if (Test-Path $item -PathType Container) {
        $files = Get-ChildItem -Path $item -Recurse -File -ErrorAction SilentlyContinue | Where-Object {
            # Exclude node_modules, .next, .git from subdirectories
            $_.FullName -notmatch "\\node_modules\\" -and
