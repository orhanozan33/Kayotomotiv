 ...
 */
export async function hashBackendPassword(password: string): Promise<string> {
 ...
 */
export async function verifyBackendPassword(plainPassword: string, hashedPassword: string): Promise<boolean> {
 ...
 */
export function verifyBackendToken(token: string): any {
  if (!process.env.JWT_SECRET) {
    throw new Error('JWT_SECRET environment variable gerekli');
  }
